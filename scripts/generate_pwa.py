...
def time_block(item: TripItem) -> str:
    """时间轴显示：第一行开始时间，↓ 分隔，第二行结束时间；无结束时单行。"""
    if not item.start:
        return '<span class="t">待定</span>'
    start = item.start.strftime("%H:%M")
    if not item.end:
        return f'<span class="t">{esc(start)}</span>'
    return (
        f'<span class="t">{esc(start)}</span>'
        f'<span class="sep">↓</span>'
        f'<span class="t">{esc(item.end.strftime("%H:%M"))}</span>'
    )
...
def render_item(item: TripItem) -> str:
    ...
    return f'''<article class="item type-{esc(kind)}{' todo' if todo else ''}" data-todo="{'1' if todo else '0'}" data-start="{item.day.isoformat()}T{item.start.strftime('%H:%M') if item.start else ''}" data-end="{item.day.isoformat()}T{item.end.strftime('%H:%M') if item.end else ''}">
      ...
      <div class="time">{time_block(item)}</div>
      <div class="item-body">
...
*{{box-sizing:border-box}}html{{--sticky-offset:calc(82px + env(safe-area-inset-top));scroll-behavior:smooth;scroll-padding-top:var(--sticky-offset)}}body{{margin:0;background:var(--bg);color:var(--ink);font-family:-apple-system,BlinkMacSystemFont,"Segoe UI","Microsoft YaHei",sans-serif;-webkit-text-size-adjust:100%;text-rendering:optimizeLegibility}}button,a{{font:inherit;-webkit-tap-highlight-color:transparent}}.hero{{padding:calc(28px + env(safe-area-inset-top)) max(18px,env(safe-area-inset-right)) 20px max(18px,env(safe-area-inset-left));background:linear-gradient(135deg,#173b73,#2563eb);color:#fff}}.hero-inner,main{{max-width:760px;margin:auto}}.eyebrow{{margin:0 0 7px;font-size:12px;opacity:.76}}h1{{font-size:25px;line-height:1.25;margin:0}}.date-range{{margin:8px 0 0;opacity:.86;font-size:14px}}.stats{{display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-top:20px}}.stat{{padding:13px 14px;border:1px solid rgba(255,255,255,.18);border-radius:14px;background:rgba(255,255,255,.12);backdrop-filter:blur(6px)}}.stat span{{display:block;font-size:12px;opacity:.8}}.stat strong{{display:block;margin-top:3px;font-size:20px}}.toolbar{{position:sticky;z-index:20;top:0;background:rgba(244,247,251,.94);border-bottom:1px solid var(--line);-webkit-backdrop-filter:blur(12px);backdrop-filter:blur(12px)}}.toolbar-inner{{display:flex;align-items:center;gap:10px;max-width:760px;margin:auto;padding:9px max(14px,env(safe-area-inset-right)) 9px max(14px,env(safe-area-inset-left))}}.day-nav{{display:flex;gap:7px;overflow-x:auto;scrollbar-width:none;flex:1}}.day-nav::-webkit-scrollbar{{display:none}}.day-nav a{{flex:0 0 46px;min-height:44px;text-align:center;text-decoration:none;color:#344054;background:#fff;border:1px solid var(--line);border-radius:11px;padding:5px 4px}}.day-nav b,.day-nav span{{display:block}}.day-nav b{{font-size:16px}}.day-nav span{{font-size:10px;color:var(--muted)}}.day-nav a.active{{color:#fff;background:var(--primary);border-color:var(--primary)}}.day-nav a.active span{{color:#dbeafe}}main{{padding:16px max(14px,env(safe-area-inset-right)) calc(50px + env(safe-area-inset-bottom)) max(14px,env(safe-area-inset-left))}}.day{{margin:0 0 18px}}.day-header{{display:flex;align-items:end;justify-content:space-between;gap:12px;margin-bottom:9px;padding:0 2px}}.day-header h2{{font-size:17px;margin:2px 0 0}}.day-date{{color:var(--primary);font-weight:800;font-size:12px}}.day-summary{{text-align:right;white-space:nowrap}}.day-summary strong,.day-summary span{{display:block}}.day-summary strong{{font-size:15px}}.day-summary span{{font-size:11px;color:var(--muted);margin-top:2px}}.day.today .day-header{{border-left:4px solid var(--primary);padding-left:9px}}.timeline{{position:relative}}.timeline:before{{content:"";position:absolute;left:78px;top:18px;bottom:18px;width:2px;background:var(--line)}}.item{{--type:#64748b;position:relative;display:grid;grid-template-columns:70px 1fr;gap:18px;margin-bottom:10px}}.item:before{{content:"";position:absolute;left:74px;top:20px;width:10px;height:10px;border:3px solid var(--bg);border-radius:50%;background:var(--type);z-index:1}}.time{{display:flex;flex-direction:column;align-items:flex-end;padding-top:12px;font-size:20px;font-weight:700;color:#475467;line-height:1.32;white-space:nowrap}}.time .sep{{font-size:13px;font-weight:600;color:#98a2b3;line-height:1.15;margin:1px 0}}.item-body{{min-width:0;padding:13px 13px 11px;border-radius:15px;background:var(--card);border:1px solid #e8edf4;box-shadow:var(--shadow)}}.item.todo .item-body{{border-color:#f4cf5d}}.item-heading{{display:flex;align-items:center;gap:8px}}.item-heading h3{{font-size:16px;margin:0;min-width:0}}.type{{flex:0 0 auto;padding:3px 7px;border-radius:7px;color:var(--type);background:color-mix(in srgb,var(--type) 13%,white);font-size:11px;font-weight:800}}.note{{margin-top:8px;color:#475467;line-height:1.55;font-size:13px;white-space:pre-wrap;overflow-wrap:anywhere}}.location{{margin-top:8px;padding:8px 10px;border-radius:9px;background:#edf7ff;color:#194f86;font-size:13px;font-weight:700;overflow-wrap:anywhere}}.booking{{margin-top:8px;padding:7px 9px;border-radius:8px;background:#fff8db;color:#765600;font-size:12px}}.cost{{display:flex;align-items:center;justify-content:flex-end;gap:7px;margin-top:8px}}.cost strong{{font-size:14px}}.price-detail{{font-size:11px;color:var(--muted)}}.item-footer{{display:flex;align-items:center;gap:6px;flex-wrap:wrap;margin-top:10px}}.status{{margin-right:auto;padding:4px 8px;border-radius:99px;background:#e7edf4;color:#344054;font-size:11px;font-weight:800}}.status-待购买,.status-待预约{{background:#fef0b8;color:#704d00}}.status-已购买,.status-已预约,.status-已完成{{background:#d7f5df;color:#176b32}}.status-已取消{{background:#e5e7eb;color:#6b7280}}.actions{{display:flex;gap:5px}}.actions button,.copy-item{{display:inline-flex;align-items:center;justify-content:center;min-height:36px;border:0;background:#eef4ff;color:#2457a7;border-radius:8px;padding:7px 9px;text-decoration:none;cursor:pointer;font-size:11px}}.copy-item{{background:#f2f4f7;color:#475467}}.item.current .item-body{{border:2px solid var(--primary);box-shadow:0 10px 28px rgba(37,99,235,.16)}}.item.current:after{{content:"正在进行";position:absolute;right:10px;top:-6px;padding:3px 7px;border-radius:99px;background:var(--primary);color:#fff;font-size:10px;font-weight:800}}.item.next .item-body{{border-color:#88b5f4}}.item.next:after{{content:"下一项";position:absolute;right:10px;top:-6px;padding:3px 7px;border-radius:99px;background:#dbeafe;color:#1d4f91;font-size:10px;font-weight:800}}.map-dialog{{width:min(360px,calc(100% - 28px));border:0;border-radius:18px;padding:0;box-shadow:0 24px 70px rgba(16,24,40,.28)}}.map-dialog::backdrop{{background:rgba(15,23,42,.42);backdrop-filter:blur(2px)}}.map-panel{{padding:18px}}.map-panel h2{{margin:0;font-size:18px}}.map-place{{margin:6px 0 14px;color:var(--muted);font-size:13px;overflow-wrap:anywhere}}.map-options{{display:grid;gap:8px}}.map-options button{{min-height:48px;border:1px solid var(--line);border-radius:11px;background:#fff;color:var(--ink);font-weight:700;cursor:pointer}}.map-options button.primary{{border-color:var(--primary);background:var(--primary);color:#fff}}.map-close{{width:100%;min-height:44px;margin-top:8px;border:0;background:transparent;color:var(--muted)}}.offline-badge{{position:fixed;z-index:30;right:max(12px,env(safe-area-inset-right));bottom:calc(12px + env(safe-area-inset-bottom));padding:6px 9px;border-radius:99px;background:#475467;color:#fff;font-size:11px;font-weight:700}}.type-交通{{--type:#2673d9}}.type-住宿{{--type:#7c4dcc}}.type-游览{{--type:#219653}}.type-餐饮{{--type:#dc7318}}.type-日常{{--type:#667085}}.type-购物{{--type:#d94f8a}}.type-其他{{--type:#80685b}}footer{{text-align:center;color:#98a2b3;font-size:11px;padding:8px 16px 28px}}@media(max-width:420px){{h1{{font-size:23px}}.hero{{padding-bottom:18px}}.stats{{margin-top:16px}}.stat{{padding:11px 12px}}.stat strong{{font-size:18px}}.item{{grid-template-columns:70px 1fr;gap:18px}}.timeline:before{{left:78px}}.item:before{{left:74px}}.item-body{{padding:12px 12px 10px}}.actions{{gap:4px}}}}@media(max-width:370px){{.actions a{{display:none}}}}@media(display-mode:standalone){{.toolbar{{top:env(safe-area-inset-top)}}}}@media(prefers-reduced-motion:reduce){{html{{scroll-behavior:auto}}}}.hero-title{{display:flex;align-items:center;justify-content:space-between;gap:12px}}.hero-title h1{{flex:1;min-width:0}}.theme-toggle{{flex:0 0 auto;min-height:44px;min-width:44px;border:1px solid rgba(255,255,255,.28);background:rgba(255,255,255,.16);color:#fff;border-radius:12px;font-size:17px;cursor:pointer;display:flex;align-items:center;justify-content:center;-webkit-backdrop-filter:blur(6px);backdrop-filter:blur(6px)}}[data-theme=dark]{{--bg:#0f172a;--card:#1e293b;--ink:#e2e8f0;--muted:#94a3b8;--line:#334155;--primary:#3b82f6;--shadow:0 8px 24px rgba(0,0,0,.45);color-scheme:dark}}[data-theme=dark] .toolbar{{background:rgba(15,23,42,.92)}}[data-theme=dark] .day-nav a{{background:#1e293b;color:#cbd5e1;border-color:#334155}}[data-theme=dark] .day-nav a.active{{background:var(--primary);color:#fff;border-color:var(--primary)}}[data-theme=dark] .day-nav a.active span{{color:#dbeafe}}[data-theme=dark] .item-body{{border-color:#334155}}[data-theme=dark] .time{{color:#cbd5e1}}[data-theme=dark] .time .sep{{color:#64748b}}[data-theme=dark] .note{{color:#cbd5e1}}[data-theme=dark] .location{{background:#162a3f;color:#7dd3fc}}[data-theme=dark] .booking{{background:#332b10;color:#fbbf24}}[data-theme=dark] .type{{background:color-mix(in srgb,var(--type) 24%,#1e293b)}}[data-theme=dark] .status{{background:#293548;color:#cbd5e1}}[data-theme=dark] .status-待购买,[data-theme=dark] .status-待预约{{background:#3d3416;color:#fde68a}}[data-theme=dark] .status-已购买,[data-theme=dark] .status-已预约,[data-theme=dark] .status-已完成{{background:#123c28;color:#86efac}}[data-theme=dark] .status-已取消{{background:#334155;color:#cbd5e1}}[data-theme=dark] .actions button{{background:#1e3a5f;color:#93c5fd}}[data-theme=dark] .copy-item{{background:#293548;color:#cbd5e1}}[data-theme=dark] .item.todo .item-body{{border-color:#8a6d1f}}[data-theme=dark] .item.next .item-body{{border-color:#3b82f6}}[data-theme=dark] .item.next:after{{background:#1e3a5f;color:#93c5fd}}[data-theme=dark] .price-detail{{color:#94a3b8}}[data-theme=dark] .offline-badge{{background:#334155;color:#e2e8f0}}[data-theme=dark] footer{{color:#64748b}}[data-theme=dark] .map-dialog{{background:var(--card);color:var(--ink)}}[data-theme=dark] .map-panel h2{{color:var(--ink)}}[data-theme=dark] .map-place{{color:#94a3b8}}[data-theme=dark] .map-options button{{background:#1e293b;border-color:#334155;color:var(--ink)}}[data-theme=dark] .map-options button.primary{{background:var(--primary);color:#fff;border-color:var(--primary)}}[data-theme=dark] .map-close{{color:#94a3b8}}
</style>
</head>
<body>
<header class="hero"><div class="hero-inner"><p class="eyebrow">由 Excel 自动生成 · {esc(sheet_name)}</p><div class="hero-title"><h1>{esc(title)}</h1><button id="themeToggle" class="theme-toggle" type="button" aria-label="切换浅色/深色模式" title="切换浅色/深色模式">🌙</button></div><p class="date-range">{esc(date_text)}</p><div class="stats"><div class="stat"><span>预计总消费</span><strong>{money(total_cost)}</strong></div><div class="stat"><span>待处理事项</span><strong>{todo_count}</strong></div></div></div></header>
<nav class="toolbar"><div class="toolbar-inner"><div class="day-nav">{nav}</div></div></nav>
...
(()=>{{
  ...
  if('serviceWorker' in navigator && location.protocol.startsWith('http')){{
    window.addEventListener('load',()=>{{
      navigator.serviceWorker.register('./service-worker.js',{{updateViaCache:'none'}}).catch(error=>console.warn('离线缓存注册失败',error));
    }});
  }}
}})();
</script>
